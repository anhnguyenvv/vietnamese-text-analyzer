...
def _normalize_vncore_token(token):
    return token.replace("_", " ") if isinstance(token, str) else token
...
def pos_tagging(text, model='vncorenlp'):
    ...
    if model == 'vncorenlp':        
        ...
        pos_tags = [(_normalize_vncore_token(word), tag) for sublist in pos_tags for word, tag in sublist]
